...
// Dynamic import of ImageEditorModal (SSR false)
const ImageEditorModal = dynamic(
  () => import("@/app/(website)/create-campaigs/_components/ImageEditorModal"),
  { ssr: false }
);
...
export default function CreativeStep({
  ...
  const inputStyle = "border border-[#4B4B4B] h-12 rounded-[8px] bg-[#4B4B4B] text-white placeholder:text-gray-400 focus-visible:ring-1 focus-visible:ring-[#FF6900]";
  ...
  const handleFileUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onloadend = () => {
      ...
      if (file.type.startsWith("image/")) {
        setCreative(prev => ({ ...prev, file, preview, format: "SINGLE_IMAGE" }));
        setShowEditor(true);
      } else if (file.type.startsWith("video/")) {
        setCreative(prev => ({ ...prev, file, preview, format: "VIDEO" }));
        setShowEditor(false);
      }
    };
  ...
  const publishMutation = useMutation({
    mutationFn: async () => {
      ...
      const payload = { adCreativeId: createdCreativeId, adSetId, adAccountId, pageId };
    ...
    onSuccess: () => {
      toast.dismiss();
      toast.success("Ad Published Successfully!", { description: "Your ad is now live on Facebook!", duration: 8000 });
    },
    onError: (err: any) => {
      toast.dismiss();
    ...
    <Card className="p-4 sm:p-6 lg:p-8 bg-[#2A2A2A] border-[#4B4B4B] mx-auto max-w-5xl">
      ...
      <div className="space-y-6 lg:space-y-8">
        ...
        <div>
          ...
          <Input value={creative.name} onChange={e => setCreative(prev => ({ ...prev, name: e.target.value }))} placeholder="e.g. Summer Sale 2025" className={inputStyle} />
        </div>
        ...
        <div>
          ...
          <div className="flex flex-col sm:flex-row gap-3">
            <Input value={creative.websiteUrl || ""} onChange={e => setCreative(prev => ({ ...prev, websiteUrl: e.target.value }))} placeholder="https://yourwebsite.com" className={inputStyle + " flex-1"} />
            <Button onClick={() => generateAdMutation.mutate(creative.websiteUrl || "")} disabled={generateAdMutation.isPending || !creative.websiteUrl} className="bg-[#FF6900] hover:bg-[#e85e00] h-12 px-4 sm:px-6 rounded-[8px] w-full sm:w-auto">
        ...
        {/* Media Upload */}
        <div>
          <Label className="text-white text-sm sm:text-base">Upload {creative.format === "VIDEO" ? "Video" : "Image"}</Label>
          <input type="file" accept="image/*,video/*" ref={inputRef} onChange={handleFileUpload} className="hidden" />
          {!creative.preview && (
            <div onClick={handleFileSelect} className="cursor-pointer border-2 border-dashed border-gray-500 p-8 sm:p-12 rounded-lg text-center bg-[#3A3A3A] hover:bg-[#4B4B4B] transition-colors">
              <Upload className="mx-auto mb-4 h-10 w-10 text-[#FF6900]" />
              <p className="text-white text-sm sm:text-base">Click to upload image or video</p>
            </div>
          )}
          ...
          {showEditor && creative.preview && creative.format === "SINGLE_IMAGE" && (
            <ImageEditorModal
              imageSrc={creative.preview}
              onApply={(editedFile, previewUrl) => { setCreative(prev => ({ ...prev, file: editedFile, preview: previewUrl })); setShowEditor(false); }}
              onCancel={() => { setShowEditor(false); removeFile(); }}
            />
          )}
          ...
          {creative.preview && !showEditor && (
            <div className="relative max-w-full mx-auto">
              {creative.format === "VIDEO" ? (
                <video src={creative.preview} controls className="rounded-lg shadow-lg w-full max-h-96 object-contain bg-black" />
              ) : (
                <img src={creative.preview} alt="Preview" className="rounded-lg shadow-lg w-full max-h-96 object-contain" />
              )}
              <button onClick={removeFile} className="absolute top-2 right-2 bg-black/70 hover:bg-black text-white rounded-full p-2 transition">
                <X className="h-5 w-5" />
              </button>
            </div>
          )}
        ...
        <div>
          ...
          <Select value={creative.format} onValueChange={(v: "SINGLE_IMAGE" | "VIDEO") => { setCreative(prev => ({ ...prev, format: v, file: null, preview: null })); setShowEditor(false); }}>
        ...
        <div>
          ...
          <Input value={creative.headline} onChange={e => setCreative(prev => ({ ...prev, headline: e.target.value }))} placeholder="e.g. 50% Off Everything!" className={inputStyle} />
        </div>

        {/* Primary Text */}
        <div>
          <Label className="text-white text-sm sm:text-base">Primary Text</Label>
          <textarea rows={5} value={creative.primaryText} onChange={e => setCreative(prev => ({ ...prev, primaryText: e.target.value }))} placeholder="Write an engaging ad message that grabs attention..." className="w-full border border-[#4B4B4B] bg-[#4B4B4B] text-white p-4 rounded-[8px] outline-none placeholder:text-gray-400 text-sm sm:text-base resize-none" />
        </div>

        {/* Destination URL */}
        <div>
          <Label className="text-white text-sm sm:text-base">Destination URL</Label>
          <Input value={creative.destinationUrl} onChange={e => setCreative(prev => ({ ...prev, destinationUrl: e.target.value }))} placeholder="https://yourwebsite.com/offer" className={inputStyle} />
        </div>
      </div>

      {/* Action Buttons */}
      <div className="mt-10 sm:mt-12 flex flex-col sm:flex-row justify-between gap-4">
        <Button variant="outline" onClick={prevTab} className="border-white text-white hover:bg-[#FF6900]/10 h-12 rounded-[8px] order-2 sm:order-1">
          <ChevronLeft className="mr-2 h-5 w-5" /> Back
        </Button>

        <div className="flex flex-col sm:flex-row gap-4 order-1 sm:order-2 w-full sm:w-auto">
          <Button onClick={() => createCreativeMutation.mutate()} disabled={createCreativeMutation.isPending} className="bg-[#FF6900] hover:bg-[#e85e00] h-12 px-8 rounded-[8px] w-full sm:w-auto">
            {createCreativeMutation.isPending ? "Creating..." : "Add Creative"}
          </Button>

          <Button onClick={() => publishMutation.mutate()} disabled={!creativeCreated || publishMutation.isPending} className={`h-12 px-8 rounded-[8px] flex items-center justify-center transition-all w-full sm:w-auto ${creativeCreated ? "bg-[#00D4FF] hover:bg-[#00b8e6] text-white" : "bg-gray-600 text-gray-400 cursor-not-allowed"}`}>
